...
client = conf0.connect{callback = function(res)
	...
	local items = {}
	execute{proc = conf0.browse, ref = client, type = type, callback = function(i) 
		if i.name and i.type and i.domain and not items[i.name] then
			items[i.name] = i
			execute{proc = conf0.resolve, ref = client, interface_ = i.interface_, protocol = i.protocol, name = i.name, type = i.type, domain = i.domain, callback = function(j)
				...
				if 'bonjour' == conf0.backend then
					if j.host then
						execute{proc = conf0.query, ref = client, fullname = j.host, type = conf0.types.A, class_ = conf0.classes.IN, callback = function(k)
